import { FlaskConical, GraduationCap, Hexagon, Info } from "lucide-react";
import {
  Accordion,
  AccordionContent,
  AccordionItem,
...
type BenzenePlanarPanelProps = {
  activeMode: BenzenePlanarMode;
  planeView: BenzenePlaneView;
};

export function BenzenePlanarPanel({ activeMode, planeView }: BenzenePlanarPanelProps) {
  const modeInfo = getBenzenePlanarModeInfo(activeMode);
  ...
  return (
    <aside className="flex h-full min-h-[400px] flex-col overflow-hidden rounded-3xl border border-white/50 bg-white/60 shadow-[0_8px_30px_rgb(0,0,0,0.04)] backdrop-blur-md">
      <div className="border-b border-white/40 bg-white/40 px-6 py-5 backdrop-blur-sm">
        ...
        <h1 className="mt-2 text-2xl font-bold leading-tight text-text-primary">苯环平面结构</h1>
            ...
            <AccordionItem value="info" className="rounded-2xl border border-white/60 bg-white/60 px-5 shadow-sm border-b-0 data-[state=open]:pb-2">
              ...
              <AccordionContent>
                <div className="grid gap-2 text-sm">
                  ...
                  <FactRow label="原子数" value="6 个 C + 6 个 H，共 12 个原子" />
                  <FactRow label="空间特点" value="苯环整体可视为平面正六边形" />
                  <FactRow label="代表键角" value="约 120°" />
                  {activeMode === "plane" ? (
                    <FactRow
                      label="当前视角"
                      value={planeView === "side" ? "侧视验证 12 原子共面" : "俯视观察正六边形"}
                      strong
                    />
                  ) : null}
                </div>
            ...
            <AccordionItem value="exam" className="rounded-2xl border border-primary/15 bg-primary/5 px-5 shadow-sm border-b-0 data-[state=open]:pb-2">
              ...
              <AccordionContent>
                <p className="text-sm leading-6 text-text-secondary">
                  做共面题时，先把苯环整体看作一个稳定平面，再判断取代基是否能通过单键旋转对齐到这个平面内。
                </p>
              </AccordionContent>
            </AccordionItem>
          </Accordion>
