import { FlaskConical, GraduationCap, Info, Ruler } from "lucide-react";
import {
  Accordion,
  AccordionContent,
  AccordionItem,
...
type AcetyleneLinearPanelProps = {
  activeMode: AcetyleneLinearMode;
  lineView: AcetyleneLineView;
};

export function AcetyleneLinearPanel({ activeMode, lineView }: AcetyleneLinearPanelProps) {
  const modeInfo = getAcetyleneLinearModeInfo(activeMode);
  ...
  return (
    <aside className="flex h-full min-h-[400px] flex-col overflow-hidden rounded-3xl border border-white/50 bg-white/60 shadow-[0_8px_30px_rgb(0,0,0,0.04)] backdrop-blur-md">
      <div className="border-b border-white/40 bg-white/40 px-6 py-5 backdrop-blur-sm">
        ...
        <h1 className="mt-2 text-2xl font-bold leading-tight text-text-primary">乙炔直线结构</h1>
            ...
            <AccordionItem value="info" className="rounded-2xl border border-white/60 bg-white/60 px-5 shadow-sm border-b-0 data-[state=open]:pb-2">
              ...
              <AccordionContent>
                <div className="grid gap-2 text-sm">
                  ...
                  <FactRow label="原子数" value="2 个 C + 2 个 H，共 4 个原子" />
                  <FactRow label="空间特点" value="H–C≡C–H 四原子共线" />
                  <FactRow label="杂化方式" value="sp" />
                  <FactRow label="代表键角" value="180°" />
                  {activeMode === "line" ? (
                    <FactRow
                      label="当前视角"
                      value={lineView === "side" ? "侧视验证同轴" : "正视观察直线"}
                      strong
                    />
                  ) : null}
                </div>
            ...
            <AccordionItem value="exam" className="rounded-2xl border border-primary/15 bg-primary/5 px-5 shadow-sm border-b-0 data-[state=open]:pb-2">
              ...
              <AccordionContent>
                <p className="text-sm leading-6 text-text-secondary">
                  做共线题时，优先寻找 C≡C、C≡N、CO₂ 等直线片段；共线原子一定共面，但共面原子不一定共线。
                </p>
              </AccordionContent>
            </AccordionItem>
          </Accordion>
