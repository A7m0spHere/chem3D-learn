> **归档说明**：本文件是 `docs/PROJECT_STATUS.md` 截至 2026-08-13（T-039D 收口）的全文快照，含 T-038/T-039 各阶段结果详情与 2026-07-25～08-13 全部独立验证日志。2026-08-27 整理后现行 PROJECT_STATUS.md 只保留当前事实与风险，历史原文在此存档。
...
- 极性、σ / π、有机平面和有机共面在 `xl` 以上使用大 Viewer + 304px Inspector rail；杂化 / 成键基础使用 360px 高密度 rail 容纳进度滑杆和四个附加开关。右栏依次放真实控制与折叠信息，不 sticky、不独立滚动；低于 `xl` 的全部专题都使用纵向顺序。
