...
- 基于已合并 T-039A 的 `main@5f3606acc7f1cb4399f94c967ced5acda05758e9` 创建独立分支 `codex/t039b-specialty-viewers`；覆盖极性、σ / π 键、杂化 / 离子键 / 配位键、乙烯 / 苯 / 乙炔和有机共面 10 个公开专题。
- `ModuleDetailPage` 不再渲染 7 个专题教学 Panel；所有专题改用默认折叠的精简模型信息，摘要只表达模块身份、当前模式和关键状态，展开只保留结构 / 轨道类型、当前模式、关键状态、可选关键几何值与模型边界。
- 极性、σ / π、有机平面和有机共面在 `xl` 以上使用大 Viewer + 304px Inspector rail；右栏依次放真实控制与折叠信息，不 sticky、不独立滚动。杂化 / 成键基础因包含进度滑杆和四个附加开关，保持 Viewer → 密集控制 → 折叠信息的纵向布局；低于 `xl` 的全部专题都使用纵向顺序。
...
- 验证：lint、build、logic 162 / 162、系统 Chrome 专题行为 7 / 7、跨模块状态 5 / 5、production 4 / 4 通过；1280×720 验证 304px rail，1024×768 与 390×844 验证纵向顺序、移动端两列按钮、键盘 Disclosure 和无横向溢出。Windows 未运行或更新 Darwin 快照。
...
- T-039A 已通过 PR #2 合并，merge commit 为 `5f3606acc7f1cb4399f94c967ced5acda05758e9`；旧 T-039A 分支不再承担后续开发。
...
- **T-035 已安全撤销，产品主线回到 3D 引导观察**（2026-08-03）：`1c958be` 之后没有后续功能提交依赖，现已用原生 `git revert` 删除自测类型、题目数据、判题函数、React 状态 hook、`ExamTopicQuiz`、详情页接线及两项测试。Chem3D Learn 不以在线作答、评分、重试或题库为核心；下一阶段改为 T-038“3D 引导观察与结构解释样板”。T-036 自测扩展已取消。T-034 的 XeO 公开占位清理完整保留。
...
- **T-032 产品完备度审计已完成，原自测结论已被纠正**（2026-08-01；2026-08-03 更新）：正式站点已有 32 个可交互结构模块、3 条参考顺序和 6 个考试专题详情，公开模块均由真实结构数据或专题 Viewer 承接。化学待核实项已由 T-033 收口，XeO 公开占位已由 T-034 清理；审计把“缺少站内作答反馈”判断为核心缺口的结论现已由 D-039 取代。当前主线是强化既有 3D 模型的引导观察、结构变化解释和对比理解，而不是扩张在线自测。

- **T-031 v0.1.0-rc.1 真实反馈收集与问题分级已暂停**（2026-08-01；2026-08-03 更新）：反馈表单、指南、P0–P3 和台账仍保留，但现阶段不做广泛 RC 反馈目标。待 T-038 的单个 3D 引导观察样板完成后，再由维护者和少量朋友验证能否找到观察目标、理解结构变化并说出原因；不设置人数 KPI。真实反馈仍为 0，稳定版决策未开始；`v0.1.0-rc.1` tag 与 GitHub Prerelease 不变。
...
- 开工前 `git fetch origin`；`main`、`origin/main` 同为 `1c958be`，差异 0 / 0，工作区干净。提交图确认 `1c958be` 是 HEAD，之后没有提交或功能依赖，父提交 `a705ba4` 为必须保留的 T-034。
- 使用 `git revert --no-commit 1c958be` 生成逆向差异，无冲突；删除自测专用源码与测试，并恢复 `ExamTopicDetailPage` 原有静态讲义 + 关联 3D 模块结构。
- T-036 已取消；产品方向明确为旋转、缩放、点击、突出、拆解、比较和分步动画驱动的 3D 观察与结构解释，不建设选择题、判题、分数、重试或题库主线。
- `npm run lint`、`npm run build` 通过；logic **160 / 160**；系统 Chrome Exam **8 / 8**、production **4 / 4**；Python Playwright 独立探针通过。仅有既有按需 `ThreeViewerFrame` 837.71 KB large chunk 警告。
- 未运行或更新 Darwin 快照；未修改 lockfile、发布版本、3D 模型或化学结构。
