...
> 最后更新：2026-08-28（T-041 code review 收口：无障碍补齐、死代码与孤儿基线清理、治理文档纠偏；四条待办见 TASKS.md）
...
- **任务面**：T-040「视觉基线迁移至 ubuntu CI」基线部分已落地（PR #4 已合并，78 张 `-linux.png` 进入 `main`），收口待 `verify` 模式连续两轮全绿；T-041「code review 收口」于 2026-08-28 完成低风险修复，四条待办待决策。
...
- 测试基线：logic **163 / 163**（2026-08-28 于 Windows 复跑通过）、ESLint 零警告、`tsc --noEmit` 通过；视觉基线两套——Linux **78 张** `*-linux.png` 为现行基线（CI 维护），darwin 78 张为历史遗留待清理；backend 最近记录 22 / 22。
...
- **`main` 无自动质量门禁**：push 主分支即触发 `deploy-pages.yml` 自动部署，流程内只跑 `test:pages`（不含 lint / logic / 视觉）；`visual-regression.yml` 仅 `workflow_dispatch`。视觉回归目前不拦截任何改动，见 T-041-A。
