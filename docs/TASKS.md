...
- [x] **T-039A 共享布局与普通分子**（2026-08-10）：删除 `ExplorerPanel`、步骤状态、全部 `lessonSteps` / `GuidedObservation*` 及对应测试；普通分子只保留名称 / 分子式、构型、典型键角与模型边界。PR #2 的维护者审阅最终采用桌面“大 Viewer + 240px 右侧控制栏”、移动端 Viewer → 工具栏 → 默认折叠结构信息；恢复大 Viewer 高度，右栏只放四个模型控制。当前分支 `codex/t039a-3d-first-clean`，PR #2 保持 Draft，待视觉复核与合并。
