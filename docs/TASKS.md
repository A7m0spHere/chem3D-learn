...
### T-031 v0.1.0-rc.1 小范围 Alpha 反馈

- **状态**：T-033～T-035 的恢复条件已于 2026-08-03 满足，待维护者启动小范围 Alpha。
- **目标**：由维护者和少量朋友走“晶胞均摊自测样板 + 代表性 3D 路径”，只记录真实问题，不设置人数 KPI。
- **验收标准**：
  - [ ] 至少完成一次真实样板走查，记录设备、路径、卡点和建议。
  - [ ] 反馈继续按既有 Bug / 化学 / UX 表单与 P0–P3 规则分诊，不把零 Issue 当作稳定证据。
  - [ ] 先根据真实反馈决定是否修正样板，再启动 T-036；不自动发布 `v0.1.0` 或 `rc.2`。

### T-036 扩展现有考试专题自测闭环

- **目标**：在 T-035 样板完成并经过 T-031 小范围 Alpha 后，将修正后的轻量自测结构扩展到其余 5 个现有专题详情页。
- **验收标准**：
  - [ ] 配位数、分数坐标、晶胞密度、空隙、金刚石 / 单晶硅均有与页面教学目标一致的自测。
  - [ ] 先吸收自测样板的真实反馈，再复用 T-035 组件和 typed 数据结构，不复制未验证的页面交互逻辑。
  - [ ] 化学答案与解释经核实；错误输入有明确反馈与重试路径。
  - [ ] build、lint、logic、针对性浏览器行为测试及适用视觉审核通过。
...
- **完成**：2026-08-03（Codex）
- **内容**：
  - 新增可复用 `ExamTopicQuiz` typed 契约、独立题目数据、纯判定函数、本地状态 hook 与通用 UI；详情页只按 topic id 取数据，不把 quiz 字段塞进六个专题讲义。
  - 仅为 `exam-crystal-formula` 注册 3 题，依次覆盖棱心贡献 `1/4`、`8 × 1/8 + 1 × 1 = 2`、`(8 × 1/8):(6 × 1/2)=1:3 → AB₃`。
  - 原生 radio 支持 Tab / 方向键 / Space；每次选择立即显示“回答正确/错误”和对应原因，支持逐题重试与整组重新练习，结果只保存在当前页面。
  - 反馈同时使用图标、明确文本和原因，不只依赖颜色；选项和按钮保持至少 44px 触控高度，390px 无横向溢出。
- **验证**：build / lint 通过；logic **164 / 164**；系统 Chrome Exam 相关回归 **10 / 10**；production **4 / 4**；Python Playwright 桌面键盘 + 移动端探针通过。
- **边界**：未扩展其余五个考试专题，未加入账号、数据库、持久化成绩、排行榜或大型题库；未修改 3D 模型、化学结构、版本、lockfile 或 Darwin 快照。
