...
### T-038 3D 引导观察与结构解释样板

- **目标**：选择一个已有、化学边界清楚的核心 3D 模块，建立“观察目标 → 操作提示 → 结构变化 → 原因解释 → 对比总结”的轻量样板，让旋转、缩放、点击、突出、拆解、比较或分步动画直接服务于空间结构理解。
- **当前状态**：**已完成**（2026-08-09）。T-038A 审计、T-038B NH₃ 四步数据契约、T-038C UI 消费与 T-038D 行为 / 响应式 / production / 视觉边界验收均已完成；普通分子 Viewer 的 NH₃ 是首个代表模块。详见 `docs/guided-observation/T038_VIEWER_AUDIT.md`。
- **任务拆解建议**：
  - [x] 审计现有 Viewer、Toolbar、Panel 与教学步骤，选择一个最能暴露共性问题的代表模块；推荐 NH₃，不新增冷门结构。
  - [x] 为 3～5 个观察步骤定义 typed 引导数据；NH₃ 已冻结为 4 步，每一步包含观察目标、允许的 3D 操作、应突出或切换的结构状态和简短解释，第四步带轻量比较出口。
  - [x] 复用现有 viewer 状态与分发边界，建立轻量引导 UI；NH₃ 以单一区域消费四段文案和轻量对比表，其他结构保留旧正文回退；不得引入选择题、判题、分数、重试或题库模型。
  - [x] 在 T-038D 验证键盘、移动端、课堂投影可读性，以及旋转 / 缩放 / 点击与步骤切换不会互相争抢控制；T-038C 仅完成无截图系统 Chrome 定向行为测试，T-038D 已补齐 1280×720 / 390×844、reduced-motion 与 production 断言。
  - [x] 先完成单个样板和自动化/边界验收，再决定是否提炼通用契约；本任务不批量扩展全部模块。
- **T-038 最终完成日期**：2026-08-09。
- [ ] 真实用户观察记录归入 T-039 小范围 Alpha，仍待启动；本任务不将自动化或 Windows 目检冒充真实用户证据。

### T-039 小范围 Alpha

- **状态**：待启动。T-038D 已通过，本轮不开展 Alpha；仅保留维护者与少量朋友的后续观察入口。
...
- **暂停原因**：当前更需要先把 3D 交互与结构解释组织成可验证的引导观察样板；广泛反馈目标与产品阶段不匹配。真实反馈仍为 0，不据此判断质量。
- **恢复条件**：T-038 样板完成并通过相应验证后，由维护者和少量朋友开展小范围 Alpha；重点记录学生能否找到观察目标、理解结构变化和说出原因，不设置人数 KPI。
...
### T-040 移除 Claude 专用协作入口

- **完成**：2026-08-10（Codex）
- **内容**：删除根目录 `CLAUDE.md` 与本地 `.claude/settings.local.json`，并将当前治理入口统一回 `AGENTS.md`。
- **边界**：保留历史交接与决策记录，不改写 Git 提交历史；本次清理不涉及业务代码、用户未提交改动或远程权限。
