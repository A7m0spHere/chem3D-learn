...
### T-022 有机拼装实验室：统一样式化键长标尺（苯种子 C–H）

- **状态**：待办（原三项内容中的 1、2 已在 T-021 提交批次中落地，仅剩第 3 项）
- **优先级**：中（视觉一致性；教学正确性部分已随 T-021 收口）
- **重要修正**：本条目此前记录「模板键角重写与旋转对齐尚未处理」，与代码实际不符。核对提交 `d6ea076` / `45485b8` 后确认：
  - **已完成①** 10 个模板坐标已按各自杂化重写，并统一到 `getStylizedBondLength` 标尺（含 H 0.92、重原子间 1.08）。逐项验算：甲基 C–H 相对母体方向 cos = −0.307/0.92 = −0.334 → **109.5°**，H–C–H 同为 109.5°；氨基 ≈107°；醛基 / 羰基 / 羧基 / 乙烯基的 sp² 方向 ≈120°；氰基 / 乙炔基为 180° 直线。原记录的 69–78° 失真值已不存在。
  - **已完成②** `types/organicBuilder.ts` 新增 `anchorDirection`（模板坐标系中母体所在方向，缺省 `[-1,0,0]`），`useOrganicBuilder.ts` 的 `addFragment` 用 `rotateVectorBetween`（`organicBuilderChemistry.ts`，Rodrigues 展开 + 反向 180° 特例）把它旋转对齐到真实母体方向，不再是纯平移。片段整体朝外张开，模板内部键角在任意吸附方向下保持不变。
  - 回归已固化在 `tests/logic/organic-builder-fixes.logic.spec.ts:299`「片段拼接按母体方向旋转对齐，原子互不重叠且保持模板键角」。
- **剩余内容**：统一样式化键长标尺 —— 苯种子的 C–H = 0.66（`organicBuilderSeeds.ts:64-65` 的 `hydrogenRadius 1.78` 减 `ringRadius 1.12`）与 `getStylizedBondLength` 的 0.92 不一致。把苯环上一个 H 拔下再吸附回去，它会比其余五个 H 明显长一截。
- **验收标准**：
  - [x] 每个片段接到游离碳上、补氢后，`matchBuilderBondAngles` 标注值与实际几何夹角偏差在合理范围内。
  - [x] 任意吸附方向下片段原子不与母体原子重叠。
  - [ ] 苯环 6 个 C–H 键长一致，包括拔下再吸附后的那个。
