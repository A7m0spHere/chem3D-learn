...
- **开始**：2026-07-30（Codex）
- **分支**：`docs/t-031-rc-feedback-loop`
- **目标**：为已发布的 `v0.1.0-rc.1` 建立可审计的真实试用、Issue 收集、P0–P3 分诊和稳定版 / 新 RC 决策门槛；不修改既有 tag 或 GitHub Release。
- **已完成的反馈基础设施**：
  - [x] 审计 GitHub Issues、默认标签、Issue config 与私密漏洞报告设置；审计时 Issues 为 0，并补齐 `rc-feedback`、`chemistry`、`ux`、`accessibility` 四个分类标签。
  - [x] 新增 Bug、化学内容、体验 / 可访问性三类 Issue Form 和 Issue 入口配置。
  - [x] 新增 `docs/RC_FEEDBACK.md`，覆盖建议试用路径、证据要求、P0–P3 与 `v0.1.0` / `rc.2` 门槛。
  - [x] 新增 `docs/RC_FEEDBACK_LOG.md`；只允许记录真实反馈，不创建示例或推测记录。
  - [x] README、仓库内 `v0.1.0-rc.1` Release Notes 和 QA 清单已链接反馈流程。
- **等待真实反馈**：
  - [ ] 至少有真实学生、教师或目标设备使用者完成候选版本试用。
  - [ ] 把实际 Issue / 反馈录入台账，并记录环境、证据、复现、优先级依据和处置。
  - [ ] 关闭所有 P0 / P1；对 P2 作出修复或接受决定。
  - [ ] 满足完整测试、Pages 冒烟与化学阻断门槛后，再单独决定是否发布 `v0.1.0`。
- **当前版本结论**：继续保留 `v0.1.0-rc.1`；真实反馈为 0，稳定版决策未开始，也没有发布 `rc.2` 的证据。
- **验证**：4 份 YAML 解析通过，3 份 Issue Form 深层字段 / ID / label 校验通过；本地反馈链接目标存在；frontend build / lint 通过。未运行视觉回归，未更新快照。
