...
- [x] **T-039A 共享布局与普通分子**（2026-08-10）：详情页改为全宽 Viewer → 工具栏 → 默认折叠结构信息；删除 `ExplorerPanel`、步骤状态、全部 `lessonSteps` / `GuidedObservation*` 及对应测试；普通分子只保留名称 / 分子式、构型、典型键角与模型边界。当前分支 `codex/t039a-3d-first`，待独立 PR 审核与合并。
- [ ] **T-039B 专题展示 Viewer**：收缩极性、σ/π 键、成键基础和有机平面专题 Panel，只保留渲染控制、关键几何值与模型边界。必须在 T-039A 合并后从最新 `main` 开始。
- [ ] **T-039C 晶体与 NaCl 周期工作台**：迁移最小 `CrystalControls`，压缩晶体 / NaCl 状态信息，保持全部周期与配位算法。
...
- **后续调整**：维护者体验触发 T-039 / D-045；T-038 证据保留用于追溯，不继续扩展或恢复公开讲解层。
