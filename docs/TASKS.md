...
- **当前状态**：第一步审计已完成（2026-08-06），推荐普通分子 Viewer 的 NH₃ 作为首个代表模块；样板契约与实现仍待办。详见 `docs/guided-observation/T038_VIEWER_AUDIT.md`。
- **任务拆解建议**：
  - [x] 审计现有 Viewer、Toolbar、Panel 与教学步骤，选择一个最能暴露共性问题的代表模块；推荐 NH₃，不新增冷门结构。
  - [ ] 为 3～5 个观察步骤定义 typed 引导数据，每一步只包含观察目标、允许的 3D 操作、应突出或切换的结构状态和简短解释。
  - [ ] 复用现有 viewer 状态与分发边界，建立轻量引导 UI；不得引入选择题、判题、分数、重试或题库模型。
  - [ ] 验证键盘、移动端、课堂投影可读性，以及旋转 / 缩放 / 点击与步骤切换不会互相争抢控制。
  - [ ] 先完成单个样板和真实观察记录，再决定是否提炼通用契约；本任务不批量扩展全部模块。
