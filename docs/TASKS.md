...
- **目标**：核实 BF₃ 缺电子表述、CaF₂ 晶胞参数和苯乙烯类近共面构象说明，消除当前源码中的 3 处 `TODO-CHEM-VERIFY`。
- **验收标准**：
  - [ ] 每条结论使用教材、同行评审文献或可追溯结构数据库，不以项目测试充当化学来源。
  - [ ] 新增或扩展化学核验文档，建立来源—可见文案—代码—测试映射。
  - [ ] 若来源不支持现有表述，先修教学文案或模型边界，不猜测精确值。
  - [ ] build、lint、适用 logic / 浏览器行为测试通过；涉及视觉变化时不在 Windows 更新 Darwin 快照。

### T-034 清理能力扩展页的公开“建设中”入口

- **目标**：在 XeO 专题真正实现前，从公开能力扩展目录移除不可进入的“建设中”卡片，并保持专题数量、分组和链接一致。
- **验收标准**：
  - [ ] 公开 Exam 页面不再展示无法进入的专题卡片或“建设中”按钮。
  - [ ] 规划设想可以保留在治理文档，但不伪装为已交付产品内容。
  - [ ] 新增相应数据 / 页面行为测试；build、lint 通过。

### T-035 核心考试专题轻量自测样板

- **目标**：为“晶胞均摊与化学式推导”增加一个可复用的站内自测闭环，验证学生是否从观察迁移到作答。
- **验收标准**：
  - [ ] 至少 3 个短题或分步判断，覆盖位置贡献、平均占有数和最简比。
  - [ ] 作答后立即反馈正确与否，并给出简短原因；支持重试，不只用颜色表达结果。
  - [ ] 自测数据与通用 UI / 状态逻辑分离，供后续专题复用。
  - [ ] 不引入账号、数据库、分数排行榜或大型题库。
  - [ ] 支持键盘操作和移动端；build、lint、logic 与针对性 Chrome 行为测试通过。
...
- **暂停原因**：当前更需要先完成化学准确性与最小自测闭环；广泛反馈目标与产品阶段不匹配。真实反馈仍为 0，不据此判断质量。
- **恢复条件**：T-033、T-034、T-035 完成并通过相应验证后，由维护者和少量朋友开展小范围 Alpha；不设置人数 KPI，只记录真实使用问题。
