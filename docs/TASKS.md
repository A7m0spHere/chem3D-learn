...
- [x] **T-039A 共享布局与普通分子**（2026-08-10）：详情页改为全宽 Viewer → 工具栏 → 默认折叠结构信息；删除 `ExplorerPanel`、步骤状态、全部 `lessonSteps` / `GuidedObservation*` 及对应测试；普通分子只保留名称 / 分子式、构型、典型键角与模型边界。当前分支 `codex/t039a-3d-first-clean`，待独立 PR 审核与合并。
