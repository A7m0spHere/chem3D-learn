...
- **阶段状态**：T-039A 已实现并完成本地验收，PR #2 当前保持 Draft；维护者审阅最终选定的普通分子“`xl` 起效的大 Viewer + 272px 右侧控制栏”已完成。不要启动 T-039B，直到 A 合并。

## 本轮改动

- `ModuleDetailPage` 已删除 360px 教学右栏；普通分子仅在 `xl` 及以上采用左侧大 Viewer + 272px 右栏，右栏依次放置模型控制与默认折叠的结构信息；低于 `xl` 保持三段纵向排列。现有专题 Panel 暂时排列在 Viewer 下方，后续按 B / C 阶段分别收缩。
...
- PR #2 维护者审阅最终撤销高度压缩：普通分子恢复原大 Viewer 高度，`xl` 桌面四个控制按钮在 272px 右栏纵向占满，结构信息紧随按钮并在展开后使用单列事实；1024px 与移动端不启用右栏，移动端仍在 Viewer 下方按“两列按钮 → 结构信息”排列。右栏不悬浮、不 sticky、不含教学正文；专题、晶体与拼装 Viewer 不受影响。
...
- 系统 Chrome `molecule-viewer.visual.spec.ts`：6 / 6 通过；覆盖五个普通分子入口、默认折叠、键盘展开、44px 控件与独立工具栏状态；1280×720、1552×926 冻结 272px 右栏、纵向全宽按钮与右栏内单列结构信息，1024×768 冻结纵向布局，390×844 冻结纵向顺序、两列按钮与无横向溢出。
...
1. 在 PR #2 中复核 1280 / 1552 桌面、1024 中等宽度与 390 手机真实页面效果，确认 `xl` 起效的 272px 右栏和低于 `xl` 的纵向顺序符合维护者预期。
