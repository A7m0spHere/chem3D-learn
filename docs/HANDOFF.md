...
- **阶段状态**：T-039A 已实现并完成本地验收；clean 提交严格基于 `origin/main@9954522`，相对 main ahead 1 / behind 0。待普通 push 新分支并进行独立 PR 审核；不要启动 T-039B，直到 A 合并。
...
- 系统 Chrome `molecule-viewer.visual.spec.ts`：4 / 4 通过；覆盖五个普通分子入口、1280×720 全宽、390×844 无横向溢出、默认折叠、键盘展开、44px 控件与独立工具栏状态。
...
1. 复核 clean 分支、工作树、`origin/main` 和最终 Changed Files 后，普通 push `codex/t039a-3d-first-clean`。
2. 创建 T-039A 独立 PR；不要使用旧的 `codex/t039a-3d-first`，它含未合并的无关祖先。
