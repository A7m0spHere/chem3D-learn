...
- **日期**：2026-07-26
- **分支**：`main`
- **任务**：T-012 用 `CalloutLabel` 扩展 MXene viewer 的恒显引线标签（引线标签第二批，扩展第 1 个 viewer）
- **提交**：`2c5615a feat(crystal): leader-line callouts for MXene scene labels`（代码已提交；文档改动待随本次一起提交）

> ℹ️ 上一轮 HANDOFF 记录的「T-010/T-011 尚未提交、等用户确认、建议分两 commit」已过时。
> 实际 T-010（`d18b785`）、T-011（`fb6ceec`）早已按当时建议分 commit 落地；本轮开工时工作区
> 只有一份未提交的 MXene 半成品（`MxeneCell.tsx` + 新 spec），已收尾并提交为 `2c5615a`。

### 本次改动（T-012）

改 `MxeneCell.tsx` + 新增 1 个无截图测试，未新增组件（复用 T-011 的 `CalloutLabel`），未改后端、`package.json`、数据 JSON 或 Darwin 快照。

**`MxeneCell.tsx`：7 处**指向具体结构**的恒显 `<Html>` → `CalloutLabel`**

- 逐处把浮在结构上的恒显 `<Html>` 改为「锚点在结构上、标签沿 `offset` 外推到外围留白、引线连接」，每处均有中文锚点/偏移说明注释：
  - `comparison`（MAX → MXene）3：`MAX 前驱体｜Ti₃AlC₂`、`二维片层｜Ti₃C₂Tₓ`、`Al 层`。
  - `coordination`（C 六配位）2：`C 中心｜6 个 Ti 最近邻`（锚点中心 C `[0,0,0]`）、`Ti₆ 八面体轮廓`（锚点引用真实 `OCTAHEDRAL_TI_POSITIONS[5]` 顶点，非硬编码近似）。
  - `covalentNetwork`（表面端基）1：`O / OH / F 混合端基示意`。
  - `interlayerForce`（重新堆叠）1：`层间水 / 离子（示意）`。
- **保留** `<Html>` 不加引线的都是「不指向单一结构」的全局说明：工艺流程（`选择性移除 Al + 剥离 →`）、厚度方向标注、辅助线免责说明、堆叠标题、剖面推导，以及整个 `counting`（`FormulaScene`）的化学式推导链与通式；`showLabels` 门控标签与 `TerminationMarkers` 的端基 `kind` 原子标签走原逻辑，未改。
- **注意**：`Al 层` 所在 `MaxPrecursorBlock` 在场景左侧且外层 `group scale=0.72`，`offset` 特意朝远离场景中心方向、并放大补偿，才能真正推到外围留白（注释里已写明）。
- 教学文案文字零变化，未引新依赖，未改几何/图例/相机/`showLabels` 系统。

**新增测试（无截图）**

- `tests/visual/mxene-callout.visual.spec.ts`（chrome 通道 4 项）：comparison / coordination / covalentNetwork / interlayerForce 四个 viewMode 下 7 处转换标签仍可见，且标签中心相对 stage 中心归一化偏移 > 0.15（证明已外推到外围）。
...
- 新增 `mxene-callout.visual.spec.ts`（`PLAYWRIGHT_CHANNEL=chrome`）：**4 / 4 通过**。
- `git status`：提交前工作区只含 `MxeneCell.tsx` + 新 spec，无 lockfile / 缓存 / Darwin 快照被改写。

### 范围说明（分步策略）

引线标签扩展按 viewer 逐个提交。已完成 MOF-5（T-011）+ MXene（T-012）。**剩余 7 个 viewer**（Ren3 ~14 / MetalClosePacking ~11 / Pba ~5 / Graphite ~4 / ZnSPolytype ~3 / ZincMetal / BaTiO3 计数徽章）待续，同一 `CalloutLabel`、各自单独提交、各自配无截图冒烟。
...
- 本轮 docs 改动（DECISIONS/TASKS/PROJECT_STATUS/HANDOFF）建议作为一个单独的 docs commit 提交（代码 `2c5615a` 已先行提交）。
- 下一个 viewer 建议按剩余标签数从多到少推进：Ren3 → MetalClosePacking → Pba → Graphite → ZnSPolytype → ZincMetal → BaTiO3。每个 viewer 先核对哪些标签「指向具体结构」（转 `CalloutLabel`）、哪些是「全局说明」（保留 `<Html>`），再配一个 chrome 通道无截图冒烟，单独提交。
- 参考已落地的 MOF-5（`Mof5Cell.tsx`）与 MXene（`MxeneCell.tsx`）范式：锚点用真实几何坐标（能引用几何常量就别硬编码近似），offset 朝远离场景中心方向；对有外层 `scale` 的子 group 记得补偿。
