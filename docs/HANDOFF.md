...
- `ModuleDetailPage` 已删除 360px 教学右栏；普通分子桌面端采用左侧大 Viewer + 240px 纯控制右栏，结构信息位于双栏下方。现有专题 Panel 暂时排列在 Viewer 下方，后续按 B / C 阶段分别收缩。
...
- PR #2 维护者审阅最终撤销高度压缩：普通分子恢复原大 Viewer 高度，桌面四个控制按钮在 240px 右栏纵向占满；移动端仍在 Viewer 下方两列排列。右栏不悬浮、不 sticky、不含结构信息或教学文字；专题、晶体与拼装 Viewer 不受影响。
...
- 系统 Chrome `molecule-viewer.visual.spec.ts`：5 / 5 通过；覆盖五个普通分子入口、默认折叠、键盘展开、44px 控件与独立工具栏状态；1280×720、1552×926 冻结 240px 右栏和纵向全宽按钮，390×844 冻结纵向顺序、两列按钮与无横向溢出。
...
1. 在 PR #2 中复核桌面与手机真实页面效果，确认大 Viewer、240px 控制右栏和移动端两列按钮符合维护者预期。
