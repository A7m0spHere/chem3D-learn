...
## D-044 T-039 采用全站 3D-first 收缩，详细讲解不再作为产品竞争力
...
- **T-038 历史**：D-040～D-043 与 T-038 的 SHA、日期和验收证据保留为当时事实；D-044 只取代其后续产品方向，不追溯改写历史。

## D-045 晶体用最小控制契约与折叠事实替代教学面板

- **日期**：2026-08-13（Codex，T-039C）
- **决定**：
  1. 17 个公开晶体结构统一增加 `crystalControls`，运行时模式与空隙阶段只包含稳定 ID 和短标签；Viewer 标题由化学式、短模式标签和必要阶段标签组成，不再读取长 `CrystalTeaching` 标题 / 正文。
  2. `CrystalInfo` 成为共享晶体信息 Disclosure 的事实来源，只保留结构类型、晶格 / 教学模型、配位、晶胞计数与组成说明。删除公开 `CrystalKnowledgePanel`、观察顺序、常见误区、教学提示和长模式讲解。
  3. 晶体模块保持全宽 Viewer → 模式工具栏 → 默认折叠信息，不复制 T-039A/B 的右侧 Inspector。空隙阶段是真正驱动 3D 的高频状态，因此在进入空隙模式后作为首层按钮呈现。
  4. NaCl 周期模式把 N=1/2/3 与选择 / 六配位 / 隔离保持首层；晶胞边框归入单层二级 Disclosure，周期计数归入默认折叠信息。完整 `siteId + periodicImageShift`、ghost、`8N³` 独立位点和 `(2N+1)³` 显示实例语义不变。
  5. 为降低一次迁移风险，旧 `CrystalTeaching` 类型与 JSON 字段只暂留兼容，生产 UI 已无消费者；T-039D 在删除最终无消费者字段时一并清理。
- **理由**：晶体模式数量多，但真正需要始终可见的是改变 3D 状态的短操作。长教学面板在桌面和移动端都显著拉长页面，并让重复说明与 3D Viewer 竞争；完整移除事实又会损害课堂核对，因此采用“短控制 + 默认折叠事实 + 必要实时状态”的渐进式披露层级。
- **验证与边界**：没有改 3D 几何、原子 / 离子坐标、配位与周期算法、相机或渲染参数；Windows 系统 Chrome 六档视口无横向溢出，晶体 / NaCl / 普通分子 / 专题回归通过，Darwin 快照未更新并留给 T-039D 在 macOS 集中审核。

## D-046 拼装反馈与目录采用“短决策层 + 按需诊断”完成 T-039 收口

- **日期**：2026-08-13（Codex，T-039D）
- **决定**：
  1. Organic Builder 保留常驻的分子式、名称、式量、片段数、价态进度和首条验证问题；键角匹配、完整价态问题、系统命名说明、识别片段与模型边界合并进一个默认折叠的诊断 Disclosure。
  2. 不修改 `useOrganicBuilder`、拼装化学 / 命名 / 几何模块或 R3F Canvas；原子、键、片段、拖动、相机和历史操作仍是首层任务。
  3. Modules 卡片只承担“是什么、能否交互、看什么、为何打开”的目录决策；Paths 每条路线只保留一条理解顺序，不再重复自学文章。公开模块数量、分组、路由和 Exam 内容不变。
